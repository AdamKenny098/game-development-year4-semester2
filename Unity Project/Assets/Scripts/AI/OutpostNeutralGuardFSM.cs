...
public class OutpostNeutralGuardFSM : MonoBehaviour
{
    ...
    private void MoveToNextPatrolPoint()
    {
        ...
        while (attempts < patrolPoints.Length)
        {
            ...
            Debug.LogWarning($"[OutpostNeutralGuardFSM] Patrol point {point.name} is not close enough to the NavMesh.");
        }
    ...
    private void AutoFindCapturePointIfMissing()
    {
        if (capturePoint != null)
            return;
        ...
        GameObject zone = GameObject.Find("CaptureZone_Trigger");

        if (zone != null)
            capturePoint = zone.transform;
    }
    ...
    private void SnapToNavMeshIfNeeded()
    {
        ...
        {
            Debug.LogWarning($"[OutpostNeutralGuardFSM] {name} could not find nearby NavMesh.");
        }
    }
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        if (capturePoint != null)
        {
            ...
            Gizmos.DrawWireSphere(capturePoint.position, playerNearPointDistance);
        }

        if (hasPatrolDestination)
        {
            Gizmos.color = Color.magenta;
            Gizmos.DrawSphere(currentPatrolDestination, 0.35f);
        }
    }
}